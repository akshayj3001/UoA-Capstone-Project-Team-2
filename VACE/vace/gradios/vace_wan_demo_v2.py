...
from audio_to_video import generate_prompt_from_audio_input, generate_scene_wise_prompt_from_audio_input
...
#        output_height, output_width, frame_rate, num_frames = int(output_height), int(output_width), int(frame_rate), int(num_frames)
        ...
        if input_audio:
            prompt = generate_scene_wise_prompt_from_audio_input(input_audio)
            print(prompt)
      
        if isinstance(prompt, list) and all(isinstance(p, dict) and "generation_prompt" in p for p in prompt):
            prompts = [p["generation_prompt"] for p in prompt]
        else:
            prompts = [prompt]
