...
from audio_to_video import generate_prompt_from_audio_input, generate_scene_wise_prompt_from_audio_input, generate_scene_wise_story_prompt_from_audio_input
...
#        output_height, output_width, frame_rate, num_frames = int(output_height), int(output_width), int(frame_rate), int(num_frames)
        ...
        if input_audio:
            prompt = generate_scene_wise_story_prompt_from_audio_input(input_audio)
            print(prompt)
      
        if isinstance(prompt, list) and all(isinstance(p, dict) and "Generation Prompt" in p for p in prompt):
            prompts = [p["Generation Prompt"] for p in prompt]
        else:
            prompts = [prompt]
        ...
        for scene_prompt in prompts:
            ...
            video_frames = (torch.clamp(video / 2 + 0.5, min=0.0, max=1.0)
                            .permute(1, 2, 3, 0) * 255).cpu().numpy().astype(np.uint8)
            video_segments.append(video_frames)

        # Concatenate all video frames along the time axis
        merged_frames = np.concatenate(video_segments, axis=0)

        name = '{0:%Y%m%d%H%M%S}'.format(datetime.datetime.now())
        video_path = os.path.join(self.save_dir, f'cur_gallery_{name}_merged.mp4')
